import React from 'react'
import port01 from '../assets/img/port01.jpg'
import port02 from '../assets/img/port02.jpg'
import port03 from '../assets/img/port03.jpg'
import port04 from '../assets/img/port04.jpg'
import port05 from '../assets/img/port05.jpg'
import port06 from '../assets/img/port06.jpg'
import port07 from '../assets/img/port07.jpg'
import port08 from '../assets/img/port08.jpg'
import port09 from '../assets/img/port09.jpg'
import port10 from '../assets/img/port10.jpg'

const Port = () => {
    return (
        <section id="port">
            <div class="port__inner">
                <div class="port__title">
                    portfolio <em>포폴 작업물</em>
                </div>
                <div class="port__wrap">
                    <div class="port__item p1">
                        <span class="num">01.</span>
                        <a href="/">
                            <img src={port01} alt="" rel="noreferrer noopener" />
                        </a>
                        <h3 class="title">리액트를 이용한 포트폴리오</h3>
                        <p class="desc">
                            이 사이트는 정말 인상적인 포트폴리오입니다
                            특히 스무스한 효과와 가로 므드드 높은 쿠러리티를 자랑하빈다
                            디테일과 꼼꼼함이 넘치는 포트폴리오는 개발자의 뛰어난 능력을 엿볼 수 있습니다
                            gsap와 react.js를 이용하여 사이트를 표현한 것은 기술적인 능력과 창의성을 강조
                            세심한 코딩과 디테일한 작업으로 그이 개발 감각과 능력이
                        </p>
                    </div>
                    <div class="port__item p2">
                        <span class="num">02.</span>
                        <a href="/">
                            <img src={port02} alt="" rel="noreferrer noopener" />
                        </a>
                        <h3 class="title">리액트를 이용한 포트폴리오</h3>
                        <p class="desc">
                            이 사이트는 정말 인상적인 포트폴리오입니다
                            특히 스무스한 효과와 가로 므드드 높은 쿠러리티를 자랑하빈다
                            디테일과 꼼꼼함이 넘치는 포트폴리오는 개발자의 뛰어난 능력을 엿볼 수 있습니다
                            gsap와 react.js를 이용하여 사이트를 표현한 것은 기술적인 능력과 창의성을 강조
                            세심한 코딩과 디테일한 작업으로 그이 개발 감각과 능력이
                        </p>
                    </div>
                    <div class="port__item p3">
                        <span class="num">03.</span>
                        <a href="/">
                            <img src={port03} alt="" rel="noreferrer noopener" />
                        </a>
                        <h3 class="title">리액트를 이용한 포트폴리오</h3>
                        <p class="desc">
                            이 사이트는 정말 인상적인 포트폴리오입니다
                            특히 스무스한 효과와 가로 므드드 높은 쿠러리티를 자랑하빈다
                            디테일과 꼼꼼함이 넘치는 포트폴리오는 개발자의 뛰어난 능력을 엿볼 수 있습니다
                            gsap와 react.js를 이용하여 사이트를 표현한 것은 기술적인 능력과 창의성을 강조
                            세심한 코딩과 디테일한 작업으로 그이 개발 감각과 능력이
                        </p>
                    </div>
                    <div class="port__item p4">
                        <span class="num">04.</span>
                        <a href="/">
                            <img src={port04} alt="" rel="noreferrer noopener" />
                        </a>
                        <h3 class="title">리액트를 이용한 포트폴리오</h3>
                        <p class="desc">
                            이 사이트는 정말 인상적인 포트폴리오입니다
                            특히 스무스한 효과와 가로 므드드 높은 쿠러리티를 자랑하빈다
                            디테일과 꼼꼼함이 넘치는 포트폴리오는 개발자의 뛰어난 능력을 엿볼 수 있습니다
                            gsap와 react.js를 이용하여 사이트를 표현한 것은 기술적인 능력과 창의성을 강조
                            세심한 코딩과 디테일한 작업으로 그이 개발 감각과 능력이
                        </p>
                    </div>
                    <div class="port__item p5">
                        <span class="num">05.</span>
                        <a href="/">
                            <img src={port05} alt="" rel="noreferrer noopener" />
                        </a>
                        <h3 class="title">리액트를 이용한 포트폴리오</h3>
                        <p class="desc">
                            이 사이트는 정말 인상적인 포트폴리오입니다
                            특히 스무스한 효과와 가로 므드드 높은 쿠러리티를 자랑하빈다
                            디테일과 꼼꼼함이 넘치는 포트폴리오는 개발자의 뛰어난 능력을 엿볼 수 있습니다
                            gsap와 react.js를 이용하여 사이트를 표현한 것은 기술적인 능력과 창의성을 강조
                            세심한 코딩과 디테일한 작업으로 그이 개발 감각과 능력이
                        </p>
                    </div>
                    <div class="port__item p6">
                        <span class="num">06.</span>
                        <a href="/">
                            <img src={port06} alt="" rel="noreferrer noopener" />
                        </a>
                        <h3 class="title">리액트를 이용한 포트폴리오</h3>
                        <p class="desc">
                            이 사이트는 정말 인상적인 포트폴리오입니다
                            특히 스무스한 효과와 가로 므드드 높은 쿠러리티를 자랑하빈다
                            디테일과 꼼꼼함이 넘치는 포트폴리오는 개발자의 뛰어난 능력을 엿볼 수 있습니다
                            gsap와 react.js를 이용하여 사이트를 표현한 것은 기술적인 능력과 창의성을 강조
                            세심한 코딩과 디테일한 작업으로 그이 개발 감각과 능력이
                        </p>
                    </div>
                    <div class="port__item p7">
                        <span class="num">07.</span>
                        <a href="/">
                            <img src={port07} alt="" rel="noreferrer noopener" />
                        </a>
                        <h3 class="title">리액트를 이용한 포트폴리오</h3>
                        <p class="desc">
                            이 사이트는 정말 인상적인 포트폴리오입니다
                            특히 스무스한 효과와 가로 므드드 높은 쿠러리티를 자랑하빈다
                            디테일과 꼼꼼함이 넘치는 포트폴리오는 개발자의 뛰어난 능력을 엿볼 수 있습니다
                            gsap와 react.js를 이용하여 사이트를 표현한 것은 기술적인 능력과 창의성을 강조
                            세심한 코딩과 디테일한 작업으로 그이 개발 감각과 능력이
                        </p>
                    </div>
                    <div class="port__item p8">
                        <span class="num">08.</span>
                        <a href="/">
                            <img src={port08} alt="" rel="noreferrer noopener" />
                        </a>
                        <h3 class="title">리액트를 이용한 포트폴리오</h3>
                        <p class="desc">
                            이 사이트는 정말 인상적인 포트폴리오입니다
                            특히 스무스한 효과와 가로 므드드 높은 쿠러리티를 자랑하빈다
                            디테일과 꼼꼼함이 넘치는 포트폴리오는 개발자의 뛰어난 능력을 엿볼 수 있습니다
                            gsap와 react.js를 이용하여 사이트를 표현한 것은 기술적인 능력과 창의성을 강조
                            세심한 코딩과 디테일한 작업으로 그이 개발 감각과 능력이
                        </p>
                    </div>
                    <div class="port__item p9">
                        <span class="num">09.</span>
                        <a href="/">
                            <img src={port09} alt="" rel="noreferrer noopener" />
                        </a>
                        <h3 class="title">리액트를 이용한 포트폴리오</h3>
                        <p class="desc">
                            이 사이트는 정말 인상적인 포트폴리오입니다
                            특히 스무스한 효과와 가로 므드드 높은 쿠러리티를 자랑하빈다
                            디테일과 꼼꼼함이 넘치는 포트폴리오는 개발자의 뛰어난 능력을 엿볼 수 있습니다
                            gsap와 react.js를 이용하여 사이트를 표현한 것은 기술적인 능력과 창의성을 강조
                            세심한 코딩과 디테일한 작업으로 그이 개발 감각과 능력이
                        </p>
                    </div>
                    <div class="port__item p10">
                        <span class="num">01.</span>
                        <a href="/">
                            <img src={port10} alt="" rel="noreferrer noopener" />
                        </a>
                        <h3 class="title">리액트를 이용한 포트폴리오</h3>
                        <p class="desc">
                            이 사이트는 정말 인상적인 포트폴리오입니다
                            특히 스무스한 효과와 가로 므드드 높은 쿠러리티를 자랑하빈다
                            디테일과 꼼꼼함이 넘치는 포트폴리오는 개발자의 뛰어난 능력을 엿볼 수 있습니다
                            gsap와 react.js를 이용하여 사이트를 표현한 것은 기술적인 능력과 창의성을 강조
                            세심한 코딩과 디테일한 작업으로 그이 개발 감각과 능력이
                        </p>
                    </div>
                </div>
            </div>
        </section>
    )
}

export default Port